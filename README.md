...
The portal features an 8-step dynamic onboarding profiling system that tracks:
$$\text{Login} \rightarrow \text{OTP Verification} \rightarrow \text{Basic Info} \rightarrow \text{Subject Ratings} \rightarrow \text{Subject Deep Dive} \rightarrow \text{Academics} \rightarrow \text{Work Style Preferences} \rightarrow \text{Future Goals}$$
...
|
`http://localhost:8000`
 — Central DB, Auth, Onboarding & Chat API 
|
|
**
Psychometric Frontend
**
 (
`aptitude-frontend`
) 
...
|
 FastAPI, ReportLab PDF 
|
`http://localhost:8001`
 — RIASEC scoring & PDF report generator 
|
|
**
Sandbox Chatbot Frontend
**
 (
`chatbot-frontend`
) 
...
|
`http://localhost:5174`
 — Standalone chatbot playground 
|
|
**
Sandbox Chatbot Backend
**
 (
`chatbot-backend`
) 
...
|
 PostgreSQL, Redis 
|
 Session and OTP storage, user profiles, and recommendations 
|
...
Beacon utilizes a unified dev runner to start all services simultaneously with color-coded log outputs.
...
> [!NOTE]
> Make sure your PostgreSQL and Redis instances are running before initiating `npm run dev`.
...
`npm run dev`
|
 Spins up the central backend/frontend, psychometric backend/frontend, and sandbox frontend/backend 
|
|
`npm run setup`
|
 Automatic setup script targeting virtual environments, pip packages, and npm dependencies 
|
|
`npm run install:all`
|
 Performs a fresh reinstall of all platform dependencies 
|
|
`npm run dev:beacon-front`
|
 Runs only the main portal frontend 
|
|
`npm run dev:beacon-back`
|
 Runs only the main portal backend 
|
---
